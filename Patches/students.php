<?php
				...
				case 'upload':
					...
					if (isset($_FILES['file'])) {
						...
						if(move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
							...
							$_DB->sql_prepare("SELECT * FROM grades WHERE gradeAssignment= ? AND gradeUser= ? LIMIT 1");
							$_DB->sql_execute(array($assignment, $user));
							if ($_DB->sql_numrows() > 0) {
								...
								$_DB->sql_prepare("INSERT INTO grades (gradeAssignment, gradeUser) VALUES ( ?, ?)");
								$_DB->sql_execute(array($assignment, $user));
								$_DB->sql_prepare("SELECT * FROM grades WHERE gradeAssignment= ? AND gradeUser= ? LIMIT 1");
								$_DB->sql_execute(array($assignment, $user));
								$gradeID = $_DB->sql_fetchrow();
							...
							$_DB->sql_prepare("UPDATE  grades SET gradeFile = ? WHERE  gradeID = ? LIMIT 1") ;
							$_DB->sql_execute(array($target_file, $gradeID));
						} else{
	...
	function get_user_courses($db) {
		...
		$res = $db->sql_prepare("SELECT * FROM courses WHERE courseID = ( SELECT course FROM studentstocourses WHERE student = ? ) ");
	...
	function set_course_template($db, $tpl, $courseID) {
		...
		foreach($assignments as $assn) {
			...
			if (!$assn['assignmentUpload']) {
				...
				$tpl->assign('g_download', '<a href="' . htmlspecialchars($grade['gradeFile'], ENT_QUOTES, 'UTF-8') . '">Download</a>');
			}
		}
